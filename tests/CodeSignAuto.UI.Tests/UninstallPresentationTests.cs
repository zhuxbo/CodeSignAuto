...
public sealed class UninstallPresentationTests
{
    ...
    [InlineData(
        ...
        "将安全移除 CodeSignAuto 主程序及其拥有的系统配置。任务历史和已签名结果将保留。")]
    [InlineData(
        "en-US",
        InstallationMode.Manual,
        "Safely removes CodeSignAuto and its owned system configuration. Job history and signed results will be preserved.")]
    [InlineData(
        "zh-CN",
        InstallationMode.Service,
        "将安全移除 CodeSignAuto 主程序、其拥有的系统配置及受控数据，包括任务历史和已签名结果。")]
    [InlineData(
        "en-US",
        InstallationMode.Service,
        "Safely removes CodeSignAuto, its owned system configuration, and controlled data, including job history and signed results.")]
    ...
    [InlineData("zh-CN", "owned_resource_mismatch", "安装状态不完整或已发生变化，无法安全卸载。重启不会修复此问题；请保留安装目录，并提供 %LocalAppData%\\CodeSignAuto\\logs\\uninstall.log。\n\n错误代码：owned_resource_mismatch")]
    [InlineData("en-US", "owned_resource_mismatch", "The installation state is incomplete or changed and cannot be removed safely. Restarting will not repair it; keep the installation directory and provide %LocalAppData%\\CodeSignAuto\\logs\\uninstall.log.\n\nError code: owned_resource_mismatch")]
