...
public sealed class DesktopApplicationCompositionTests
{
    ...
    {
        var root = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "CodeSignAuto", "manual"));
        ...
        Assert.Equal(
            ...
            Assert.Throws<InstallException>(() => Program.ValidateManualDesktopReceipt(
                manual,
                Path.GetFullPath(Path.Combine(Path.GetTempPath(), "other", "CodeSignAuto.exe")),
                root)).Code);
    }
    ...
    public void Explicit_entry_routing_preserves_cli_and_desktop_modes()
    {
        ...
        [
            ...
            (["purge-quarantine", "--manifest", "C:\\ProgramData\\CodeSignAuto.Purge.x\\manifest.json"], ApplicationEntryKind.PurgeQuarantine, false),
    ...
    public void Detached_desktop_keeps_diagnostics_in_a_bounded_local_log()
    {
        ...
        try
        {
            ...
            {
                writer.WriteLine("first diagnostic path=C:\\ProgramData\\CodeSignAuto");
    ...
    {
        var native = new System.ComponentModel.Win32Exception(5, "access denied path=C:\\ProgramData\\CodeSignAuto");
        ...
        Assert.Contains("path=C:\\ProgramData\\CodeSignAuto", diagnostic, StringComparison.Ordinal);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto-desktop-local-exit-" + Guid.NewGuid().ToString("N"));
    ...
    private static AgentConfiguration CompleteConfiguration() => new(
        "S-1-5-21-1000",
        Path.Combine(Path.GetTempPath(), "CodeSignAuto", "spool"),
    ...
    {
        var root = configuredRoot ?? Path.Combine(Path.GetTempPath(), "CodeSignAuto", "local-composition");
