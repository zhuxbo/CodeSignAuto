...
internal static class ProductionQuickSignApp
{
    ...
    public static async Task RunAsync()
    {
        ...
        Require(
            string.Equals(Path.GetFileName(appHost), "CodeSignAuto.exe", StringComparison.OrdinalIgnoreCase),
    ...
    private static void RestoreFromOwnedTray(Process process, TimeSpan timeout)
    {
        ...
        do
        {
            ...
            foreach (var icon in buttons.Where(candidate =>
                         candidate.Current.Name.StartsWith("CodeSignAuto -", StringComparison.Ordinal)))
            {
