...
public sealed class WindowsDesktopIntegrationTests
{
    ...
    public async Task Wpf_runtime_preserves_theme_icon_tray_and_opens_settings_on_its_visible_main_window()
    {
        ...
        var summary = new CodeSignAuto.Protocol.ServiceSettingsSummary(
            7080,
            CodeSignAuto.Protocol.ServiceSettingsSummary.FixedMaximumUploadBytes,
            24,
            "1.0.0");
        using var settings = new CodeSignAuto.App.UI.ViewModels.ServiceSettingsDialogViewModel(
            summary,
            ...
            inspectionTimer = new System.Windows.Threading.DispatcherTimer(
                ...
                (_, _) =>
                {
                    var dialogs = application.Windows
                        .OfType<CodeSignAuto.App.UI.Views.ServiceSettingsDialog>()
                        .ToArray();
        ...
        var dialogs = Assert.IsAssignableFrom<
            CodeSignAuto.App.UI.ViewModels.IServiceSettingsDialogServiceProvider>(runtime)
...
#if CODESIGNAUTO_WPF
    private sealed class NoopSettingsEditor(CodeSignAuto.Protocol.ServiceSettingsSummary summary)
        : CodeSignAuto.App.Commands.IServiceConfigurationEditor
    {
        public Task<CodeSignAuto.App.Commands.ServiceConfigurationEditResult> ApplyAsync(
            CodeSignAuto.App.Commands.ServiceConfigurationEditRequest request,
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            return Task.FromResult(
                new CodeSignAuto.App.Commands.ServiceConfigurationEditResult(summary, null));
        }
    }

    private sealed class SettingsAdministration(CodeSignAuto.Protocol.ServiceSettingsSummary summary)
        : CodeSignAuto.Agent.Ipc.IAgentAdministrationClient
    {
        public Task<CodeSignAuto.Protocol.JobPageResponse> GetJobPageAsync(
            CodeSignAuto.Protocol.JobPageCursor? cursor,
            CancellationToken cancellationToken) => throw new NotSupportedException();

        public Task<CodeSignAuto.Protocol.ServiceSettingsSummary> GetServiceSettingsAsync(
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
        ...
        public Task SaveOtpAsync(
            CodeSignAuto.Core.Otp.OtpauthProfile profile,
            CancellationToken cancellationToken) => throw new NotSupportedException();
    }

    private sealed class NoopClipboard : CodeSignAuto.App.UI.ViewModels.IClipboardService
    {
