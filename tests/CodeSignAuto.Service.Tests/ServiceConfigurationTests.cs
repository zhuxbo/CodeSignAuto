...
public sealed class ServiceConfigurationTests
{
    ...
    public void Executable_below_a_controlled_data_root_is_rejected(string controlledRoot)
    {
        ...
        var failure = Assert.Throws<ServiceConfigurationException>(() =>
            ...
            {
                ExecutablePath = Path.Combine(root, "bin", "CodeSignAuto.exe"),
            }));
    ...
    public void Executable_below_a_same_prefix_sibling_is_accepted()
    {
        ...
        {
            ExecutablePath = Path.Combine(sibling, "CodeSignAuto.exe"),
        });

        Assert.Equal(
            Path.Combine(sibling, "CodeSignAuto.exe"),
    ...
    public void Controlled_path_boundary_is_windows_case_insensitive_and_separator_aware()
    {
        ...
        [
            ("C:\\ProgramData\\CodeSignAuto", "c:\\PROGRAMDATA\\CODESIGNAUTO", true),
            ("C:\\ProgramData\\CodeSignAuto", "c:\\PROGRAMDATA\\CODESIGNAUTO\\bin\\CodeSignAuto.exe", true),
            ("C:\\ProgramData\\CodeSignAuto", "c:\\PROGRAMDATA\\CODESIGNAUTO-tools\\CodeSignAuto.exe", false),
        ];
    ...
    private sealed class NoopSpoolAclPolicy : CodeSignAuto.Service.Jobs.ISpoolAclPolicy
    {
    ...
    private sealed class ConfigurationFixture : IDisposable
    {
        ...
        {
            var dataRoot = System.IO.Path.Combine(_root, "CodeSignAuto");
            return new ServiceConfiguration(
                ...
                System.IO.Path.Combine(_root, "install", "CodeSignAuto.exe"),
                System.IO.Path.Combine(_root, "profile", "AppData", "Local", "CodeSignAuto", "agent.json"),
                24);
        }
