...
public sealed class RestartRecoveryTests
{
    ...
    public async Task Rejected_old_connection_after_restart_does_not_consume_attempt_two()
    {
        ...
        try
        {
            ...
            {
                Assert.True(await restarted.SendHelloAndObserveCloseAsync(new SimplySignAuto.Protocol.AgentHello(
                    SimplySignAuto.Protocol.LengthPrefixedJsonProtocol.ProtocolVersion,
