...
public sealed class ConfigureServiceCommandTests
{
    ...
    {
        var factory = new ServiceConfigurationWriterLeaseFactory($"SimplySignAuto.Tests.{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "simplysign-auto-configure-service");
        ...
        return new ServiceConfiguration(
            ...
            7080, "SimplySignAuto/v1", new string('a', 32),
            Path.Combine(root, "install", "SimplySignAuto.exe"),
    ...
    private static string ProtectedConfigurationTestPath() =>
        Path.Combine(Path.GetTempPath(), "simplysign-auto-protected-config", "service.json");
