...
public sealed class AgentDiagnosticTests
{
    ...
    public void Preserves_full_diagnostics_and_redacts_only_the_actual_totp_secret()
    {
        ...
        var process = ProcessResult.Exited(
            ...
            $"stdout email=user@example.test user=signing path=C:\\ProgramData\\CodeSignAuto token=123456 secret={secret}",
        ...
        Assert.Contains("stdout email=user@example.test user=signing path=C:\\ProgramData\\CodeSignAuto token=123456", diagnostic, StringComparison.Ordinal);
