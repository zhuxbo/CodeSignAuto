...
public sealed class ReleasePackagingContractTests
{
    ...
    public void Release_publish_contract_is_framework_dependent_and_language_closed()
    {
        ...
        var project = File.ReadAllText(Path.Combine(
            ...
            "src",
            "CodeSignAuto.App",
            "CodeSignAuto.App.csproj"));
        var setupProject = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "src",
            "CodeSignAuto.Setup",
            "CodeSignAuto.Setup.csproj"));
    ...
    public void GitHub_release_workflow_is_manual_tag_bound_and_publishes_one_or_two_installers()
    {
        ...
        Assert.Contains(
            ...
            buildContract,
            StringComparison.Ordinal);
        Assert.Equal(1, Count(workflow, "CodeSignAutoSetup-${{ inputs.version }}-win-x64.exe"));
        Assert.Equal(1, Count(workflow, "CodeSignAutoPdfSetup-${{ inputs.version }}-win-x64.exe"));
    ...
    public void GitHub_release_notes_are_deterministic_and_describe_two_artifacts_when_pdf_is_released()
    {
        ...
        var mainName = $"CodeSignAutoSetup-{version}-win-x64.exe";
        var pdfName = $"CodeSignAutoPdfSetup-{version}-win-x64.exe";
        ...
        File.WriteAllText(
            ...
            "\"changedPaths\":[\"tools/pdf-signer/CodeSignAuto_pdf_signer.py\"]}",
    ...
    public void Release_build_always_produces_main_and_conditionally_produces_pdf_setup()
    {
        ...
        Assert.Contains("CodeSignAutoSetup-$Version-win-x64.exe", script, StringComparison.Ordinal);
        Assert.Contains("CodeSignAutoPdfSetup-$Version-win-x64.exe", script, StringComparison.Ordinal);
    ...
    public void Windows_publish_payload_policy_rejects_loose_runtime_python_helper_and_other_languages()
    {
        ...
        File.WriteAllText(
            ...
            ". $BuildScript -Version '1.2.3'\n" +
            "$allowed=@('CodeSignAuto.exe','e_sqlite3.dll')\n" +
            "Assert-PublishPayloadPolicy -RelativePaths $allowed\n" +
            "$withRuntimeConfig=@('CodeSignAuto.exe','e_sqlite3.dll','CodeSignAuto.runtimeconfig.json')\n" +
            "Assert-PublishPayloadPolicy -RelativePaths $withRuntimeConfig\n" +
            "$forbidden=@('dotnet-runtime-10.0.1-win-x64.exe','windowsdesktop-runtime-10.0.1-win-x64.exe','aspnetcore-runtime-10.0.1-win-x64.exe','python.exe','python313.dll','module.pyd','base_library.zip','lib/python3.13/os.py','CodeSignAutoPdfSigner.exe','embedded-tools.json','nested/runtime/hostfxr.dll','hostpolicy.dll','coreclr.dll','clrjit.dll','CodeSignAuto.pdb','zh-Hans/CodeSignAuto.resources.dll','en/CodeSignAuto.resources.dll','fr/CodeSignAuto.resources.dll','unexpected.txt')\n" +
            "foreach($path in $forbidden){try{Assert-PublishPayloadPolicy -RelativePaths @('CodeSignAuto.exe','e_sqlite3.dll',$path);throw 'forbidden_payload_accepted'}catch{if($_.Exception.Message-cne'release_publish_payload_forbidden'){throw}}}\n" +
            "try{Assert-PublishPayloadPolicy -RelativePaths @('CodeSignAuto.exe');throw 'missing_payload_accepted'}catch{if($_.Exception.Message-cne'release_publish_payload_missing'){throw}}\n" +
    ...
    public void Release_media_signer_rejects_missing_remote_signing_parameters_before_mutation()
    {
        ...
        File.WriteAllText(Path.Combine(packageRoot, "CodeSignAuto.exe"), "unsigned-app", new UTF8Encoding(false));
        File.WriteAllText(
        ...
        try
        {
            ...
            Assert.Equal("unsigned-app", File.ReadAllText(Path.Combine(packageRoot, "CodeSignAuto.exe")));
    ...
    public void Release_managed_hashes_target_the_exact_framework_dependent_publish_inputs()
    {
        ...
        Assert.Contains("src/CodeSignAuto.App", script, StringComparison.Ordinal);
    ...
    public void Windows_managed_hash_reader_uses_all_five_exact_framework_dependent_publish_inputs()
    {
        ...
        try
        {
            ...
            File.Delete(Path.Combine(fixture.BundleRoot, "CodeSignAuto.Service.dll"));
    ...
    public void Windows_catalog_closes_nested_payload_and_detects_byte_mutation()
    {
        ...
        var files = new[]
        {
            ...
            "sbom.spdx.json",
            "README.txt",
            "zh-Hans/CodeSignAuto.resources.dll",
            "en/CodeSignAuto.resources.dll",
        };
    ...
    private static GateFixture CreateGateFixture()
    {
        ...
        File.WriteAllText(
            ...
            if ($env:SSA_FAKE_TRX_MODE -eq 'missing' -and $projectName -eq 'CodeSignAuto.UI.Tests') { exit 0 }
            $failed = if ($env:SSA_FAKE_TRX_MODE -eq 'failed' -and $projectName -eq 'CodeSignAuto.Agent.Tests') { 1 } else { 0 }
            ...
            if ($env:SSA_FAKE_TRX_MODE -eq 'extra' -and $projectName -eq 'CodeSignAuto.EndToEnd.Tests') {
                [IO.File]::WriteAllText((Join-Path $resultsRoot 'unexpected.trx'), $xml, [Text.UTF8Encoding]::new($false))
            }
            if ($env:SSA_FAKE_TRX_MODE -eq 'native-failure' -and $projectName -eq 'CodeSignAuto.Agent.Tests') { exit 7 }
            exit 0
            """,
            new UTF8Encoding(false));
        return fixture;
    ...
    private static PublishPayloadFixture CreatePublishPayloadFixture()
    {
        ...
        {
            ["CodeSignAuto.exe"] = Encoding.UTF8.GetBytes("host"),
            ["e_sqlite3.dll"] = Encoding.UTF8.GetBytes("native-sqlite"),
            ["CodeSignAuto.runtimeconfig.json"] = Encoding.UTF8.GetBytes("{}")
        };
        ...
        File.WriteAllText(
            ...
            "$expected = @('e_sqlite3.dll','CodeSignAuto.exe','CodeSignAuto.runtimeconfig.json')\n" +
    ...
    private static ManagedOutputFixture CreateManagedOutputFixture()
    {
        ...
        File.WriteAllText(Path.Combine(nonR2r, "CodeSignAuto.dll"), "non-r2r-app-decoy");
