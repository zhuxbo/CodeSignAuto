...
public sealed class EmbeddedToolExtractorTests
{
    ...
    public void Pdf_extension_paths_are_fixed_under_program_files()
    {
        ...
        Assert.Equal(@"C:\Program Files\CodeSignAuto PDF Support", paths.Root);
        Assert.Equal(@"C:\Program Files\CodeSignAuto PDF Support\extension.json", paths.Manifest);
        Assert.Equal(
            @"C:\Program Files\CodeSignAuto PDF Support\CodeSignAutoPdfSigner.exe",
            paths.Helper);
    }
    ...
    public async Task Product_wide_resolver_returns_not_installed_only_when_extension_root_is_absent()
    {
        ...
        Assert.Equal(@"C:\Program Files\CodeSignAuto PDF Support", security.Paths?.Root);
