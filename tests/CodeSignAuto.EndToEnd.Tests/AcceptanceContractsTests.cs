using CodeSignAuto.Acceptance.Contracts;
using Xunit;
...
public sealed class AcceptanceContractsTests
{
    ...
    public void Trusted_executable_rejects_a_path_swap_and_manifest_hash_mismatch()
    {
        var before = TrustedExecutableSnapshot(
            @"D:\protected-repo\CodeSignAuto.exe",
            "VOL:1:FILE:1",
            Hash('a'),
    ...
    public void Agent_task_must_keep_every_execution_field_exact_before_each_action()
    {
        var expected = new AgentTaskExecutionSnapshot(
            ...
            "HighestAvailable",
            "LogonTrigger",
            "CodeSignAuto/v1/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "CodeSignAuto/v1",
    ...
    public async Task Fresh_install_token_cleanup_retries_a_transient_Windows_delete_failure()
    {
        ...
        try
        {
            ...
            {
                IsBackground = true,
                Name = "CodeSignAuto transient delete test release",
            };
            release.Start();
    ...
    public void Trx_sanitizer_preserves_diagnostics_and_redacts_only_totp_secrets()
    {
        const string raw = """
            ...
            <TestRun id="aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" name="signer@HOST01 2026-08-09" runUser="DOMAIN\signer" xmlns="http://microsoft.com/schemas/VisualStudio/TeamTest/2010">
              ...
              <TestDefinitions><UnitTest name="Safe.Test" storage="C:\repo\bin\CodeSignAuto.EndToEnd.Tests.dll" /></TestDefinitions>
        ...
        var sanitized = TrxArtifactPolicy.Sanitize(
            raw,
            "CodeSignAuto.EndToEnd.Tests.dll",
            "HOST01",
            @"DOMAIN\signer");
        ...
        Assert.Contains("storage=\"C:\\repo\\bin\\CodeSignAuto.EndToEnd.Tests.dll\"", sanitized, StringComparison.Ordinal);
    ...
    public async Task Actual_vstest_trx_is_sanitized_before_publish_and_secret_mutation_is_rejected()
    {
        ...
        var project = Path.GetFullPath(Path.Combine(
            ...
            "..", "..", "..", "CodeSignAuto.EndToEnd.Tests.csproj"));
    ...
    public void Native_test_routing_and_acceptance_script_use_the_merged_theme_tray_case()
    {
        ...
        const string uiPrefix = "CodeSignAuto.UI.Tests.WindowsDesktopIntegrationTests.";
        ...
        Assert.Equal(42, expectedTests.Split('\n').Count(line =>
            line.TrimStart().StartsWith("'CodeSignAuto.", StringComparison.Ordinal)));
    ...
    public void Purge_hard_stop_requires_exact_identity_pre_task_termination_wrapper_resume_and_preservation()
    {
        ...
        var sources = new[] { @"C:\ProgramData\CodeSignAuto" };
        var staged = new[]
        {
            @"C:\.CodeSignAuto.quarantine.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.0\program-data",
        };
    ...
    public void Purge_hard_stop_rejects_every_false_green(string mutation)
    {
        ...
        var sources = mutation == "extra-source-target"
            ? new[] { @"C:\ProgramData\CodeSignAuto", @"C:\Users\CodeSignAutoAgent\Data" }
            : new[] { @"C:\ProgramData\CodeSignAuto" };
        var staged = mutation == "extra-source-target"
            ? new[]
            {
                @"C:\.CodeSignAuto.quarantine.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.0\program-data",
                @"C:\.CodeSignAuto.quarantine.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.0\agent-data",
            }
            : new[]
            {
                @"C:\.CodeSignAuto.quarantine.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.0\program-data",
        ...
        string firstFailure)
    {
        Assert.Equal(
            ...
            UnattendedRebootCleanupPolicy.Resolve(
                firstFailure,
                new string('a', 32),
                "CodeSignAuto/Acceptance/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/Api",
                secretStored: true,
                secretAbsent: true,
                "CodeSignAuto.Acceptance.Boot.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                taskRegistered: true,
    ...
    public void Unattended_reboot_cleanup_failure_upgrades_to_state_uncertain(string mutation)
    {
        Assert.Equal(
            ...
            UnattendedRebootCleanupPolicy.Resolve(
                ...
                new string('a', 32),
                mutation == "wrong-key"
                    ? "CodeSignAuto/Acceptance/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/Api"
                    : "CodeSignAuto/Acceptance/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/Api",
                secretStored: true,
                secretAbsent: mutation != "secret-remains",
                mutation == "wrong-task"
                    ? "CodeSignAuto.Acceptance.Boot.bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
                    : "CodeSignAuto.Acceptance.Boot.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                taskRegistered: true,
    ...
    public void Boot_resume_task_requires_exact_system_startup_owner_and_action()
    {
        var expected = new BootResumeTaskSnapshot(
            ...
            "AtStartup", "CodeSignAuto/v1", "CodeSignAuto/AcceptanceBoot/v1", 900, 1);
    ...
    [Theory]
    [InlineData("CodeSignAuto.Agent.Tests.SigningWorkerTests.Holds_input_handle_without_write_or_delete_sharing_until_signer_finishes")]
    [InlineData("CodeSignAuto.EndToEnd.Tests.InstallCommandTests.Windows_executor_creates_verifies_and_rolls_back_temporary_native_resources")]
    [InlineData("CodeSignAuto.Service.Tests.SpoolStoreTests.Windows_local_upload_hardlink_rejection_preserves_external_hash_owner_and_dacl")]
    [InlineData("")]
    ...
    private static AcceptanceTaskSnapshot ExpectedTask() => new(
        "CodeSignAuto Acceptance owner=" + new string('a', 32),
        Hash('A'),
        "InteractiveToken",
        "Highest",
        Hash('B'),
