...
public sealed class InstallCommandTests
{
    ...
    public async Task Manual_install_plan_contains_only_receipt_registration_shortcut_and_verification()
    {
        ...
        Assert.Equal(
            Path.Combine(fixture.LocalApplicationData, "CodeSignAuto", "manual"),
    ...
    public async Task Setup_composes_preflight_managed_user_profile_configuration_and_existing_install()
    {
        ...
        Assert.Equal("CodeSignAutoAgent", provisioner.RequestedUserName);
        Assert.Equal("TEST\\CodeSignAutoAgent", installer.SigningUser);
    ...
    public async Task Windows_media_stager_holds_source_snapshot_copies_flushes_promotes_and_authorizes_exact_sid()
    {
        ...
        Assert.Equal(
            ["CodeSignAuto.exe", "docs/readme.txt", "install-prerequisites.ps1", "release-files.cat"],
    ...
    public async Task Windows_media_stager_rejects_existing_target_before_mutation()
    {
        ...
        Assert.False(Directory.Exists(Path.Combine(fixture.ProgramFilesRoot, "CodeSignAuto.part-testnonce")));
    }
    ...
    public void Install_media_publisher_identity_normalizes_thumbprint_case_but_rejects_multiple_signers()
    {
        ...
        var executablePath = Path.Combine(mediaRoot, "CodeSignAuto.exe");
    ...
    public void App_assembly_name_matches_the_windows_service_executable_contract()
    {
        ...
        Assert.Equal("CodeSignAuto", assembly.GetName().Name);
        Assert.Equal("CodeSignAuto.dll", Path.GetFileName(assembly.Location));
        Assert.Equal(
            Path.Combine(Path.GetDirectoryName(assembly.Location)!, "CodeSignAuto.exe"),
            ServiceConfigurationLoader.Validate(fixture.ServiceConfiguration() with
            {
                ExecutablePath = Path.Combine(Path.GetDirectoryName(assembly.Location)!, "CodeSignAuto.exe"),
            }).ExecutablePath);
    }
    ...
    public async Task Install_command_maps_service_configuration_failure_to_a_stable_error()
    {
        ...
        var environment = new RecordingInstallEnvironment(
            fixture,
            Path.Combine(fixture.Root, "CodeSignAuto.App.exe"));
    ...
    public void Installation_receipt_codec_round_trips_and_rejects_non_exact_shapes()
    {
        var receipt = new InstallationReceipt(
            ...
            InstallFixture.SigningSid,
            Path.GetFullPath(Path.Combine(Path.GetTempPath(), "installed", "CodeSignAuto.exe")),
            UserDataRoot: null);
    ...
    public async Task Windows_receipt_store_round_trips_admin_only_file_and_refuses_drifted_delete()
    {
        ...
        var receipt = new InstallationReceipt(
            ...
            "0123456789abcdef0123456789abcdef",
            currentSid.Value,
            Path.GetFullPath(Path.Combine(root, "bin", "CodeSignAuto.exe")),
            UserDataRoot: null);
        try
        {
    ...
    public async Task Install_plan_creates_local_system_service_and_exact_interactive_sid_task()
    {
        ...
        Assert.Equal("CodeSignAuto", productRegistration.DisplayName);
        Assert.Equal(ApplicationVersion.ReadIdentity(typeof(InstallCommand).Assembly), productRegistration.DisplayVersion);
        Assert.Equal("CodeSignAuto", productRegistration.Publisher);
        ...
        Assert.Equal(
            Path.Combine(fixture.CommonDesktopDirectory, "CodeSignAuto.lnk"),
            shortcut.Path);
    ...
    [Theory]
    [InlineData(@"C:\Program Files\CodeSignAuto\CodeSignAuto.exe", true)]
    [InlineData(@"C:\Program Files\CodeSignAuto\other.exe", false)]
    [InlineData(@"C:\Program Files\CodeSignAuto\nested\CodeSignAuto.exe", false)]
    [InlineData(@"C:\Users\Administrator\Desktop\CodeSignAuto\CodeSignAuto.exe", false)]
    ...
    public async Task Install_plan_rejects_an_executable_below_a_controlled_data_root(string controlledRoot)
    {
        ...
        var root = controlledRoot == "data"
            ? fixture.DataRoot
            : Path.Combine(fixture.LocalApplicationData, "CodeSignAuto");
        var planner = new InstallPlanner(new RecordingInstallEnvironment(
            fixture,
            Path.Combine(root, "bin", "CodeSignAuto.exe")));
    ...
    public async Task Registered_task_rollback_stops_and_confirms_before_delete()
    {
        ...
        await new WindowsOwnedTaskRollback(runner, lookup, runtime)
            .ExecuteAsync("CodeSignAuto.Agent", CancellationToken.None);
    ...
    public async Task Registered_service_rollback_stops_and_confirms_before_delete()
    {
        ...
        await new WindowsOwnedServiceRollback(runner, lookup, runtime)
            .ExecuteAsync("CodeSignAuto.Service", CancellationToken.None);
    ...
    public void Windows_service_ownership_verification_uses_the_installed_non_delayed_start_policy()
    {
        ...
        var expected = WindowsInstallRollbackOwnershipVerifier.CreateExpectedService(
            new StartAndVerifyWindowsService("CodeSignAuto.Service", marker),
            configuration,
            marker);
    ...
    public async Task Windows_desktop_shortcut_is_verified_and_exact_owned_rollback_removes_it()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Shortcut.Tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        var target = Path.Combine(root, "CodeSignAuto.exe");
        var shortcutPath = Path.Combine(root, "CodeSignAuto.lnk");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Install.Owner", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
    ...
    public void Installed_spool_acl_remains_valid_after_the_service_protects_its_root()
    {
        var root = Path.Combine(
            Path.GetTempPath(),
            "CodeSignAuto.SpoolAcl.Contract",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        var signingUser = WindowsIdentity.GetCurrent().User!;
        try
    ...
    public async Task Setup_tools_acl_snapshot_is_accepted_by_the_optional_tool_resolver_security_contract()
    {
        ...
        var root = Path.Combine(
            Path.GetTempPath(),
            "CodeSignAuto.OptionalToolsAcl.Contract",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
    ...
    public async Task Windows_executor_creates_verifies_and_rolls_back_temporary_native_resources()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Install.Native", suffix);
        ...
        var actions = new InstallAction[]
        {
            ...
            new CreateWindowsService($"CodeSignAuto.Install.{suffix}", "LocalSystem", executable, ["service"], true, [5, 15, 60], TestInstallOwnerMarker),
            new CreateInteractiveLogonTask($"CodeSignAuto.Install.{suffix}", account, sid, executable, ["agent", "--background"], "InteractiveToken", true, true, TestInstallOwnerMarker),
            new CreateOwnedFirewallRule($"CodeSignAuto Install {suffix}", TestInstallOwnerMarker, 47443),
        };
        ...
        try
        {
                       ...
                       writable: true)!)
            {
                key.SetValue(
                    ...
                    "CodeSignAuto/v1/ffffffffffffffffffffffffffffffff",
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (current is not null &&
            !File.Exists(Path.Combine(current.FullName, "CodeSignAuto.sln")))
        {
    ...
    private sealed class RecordingInstallMediaStager(List<string> events) : IInstallMediaStager
    {
        ...
        public Task<InstallMediaPlan> PlanAsync(CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(new InstallMediaPlan(
                @"D:\release",
                @"C:\Program Files\CodeSignAuto.part-test",
    ...
    private sealed class RecordingInstallMediaVerifier : IInstallMediaVerifier
    {
        ...
        {
            Assert.EndsWith("CodeSignAuto.exe", executablePath, StringComparison.OrdinalIgnoreCase);
    ...
    private sealed class WindowsInstallMediaFixture : IDisposable
    {
        ...
        {
            var suffix = Guid.NewGuid().ToString("N");
            var source = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Media.Source", suffix);
            var programFiles = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
                "CodeSignAuto.Media.ProgramFiles." + suffix);
            Directory.CreateDirectory(Path.Combine(source, "docs"));
            File.WriteAllBytes(Path.Combine(source, "CodeSignAuto.exe"), "signed-exe"u8.ToArray());
            File.WriteAllText(
    ...
    private sealed class RecordingSetupResourceReservation(List<string> events) : ISetupResourceReservation
    {
        ...
        public string Reserve()
        {
            ...
            return @"C:\ProgramData\CodeSignAuto";
        }
    ...
    {
        private const string Owner = "CodeSignAuto/v1/0123456789abcdef0123456789abcdef";
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(new AgentUserProvisionReadback(
                ...
                OwnerMarker: _failure switch
                {
                    ...
                    AutoLogonContractFailure.OwnerInvalid => "CodeSignAuto/v1/not-an-owner",
                    _ => Owner,
                }));
        }
    ...
    private sealed class RecordingUninstallEnvironment(ServiceConfiguration configuration) : IUninstallEnvironment
    {
        ...
        public string CommonDesktopDirectory => Path.Combine(
            Path.GetTempPath(), "CodeSignAuto.Tests", "PublicDesktop");
    ...
    private sealed class InstallFixture : IDisposable
    {
        ...
        public string Root { get; } = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Tests", Guid.NewGuid().ToString("N"));
        public string ProgramDataRoot => Path.Combine(Root, "ProgramData");
        public string DataRoot => Path.Combine(ProgramDataRoot, "CodeSignAuto");
        ...
        public string ExecutablePath => Path.Combine(Root, "install", "CodeSignAuto.exe");
        ...
        public ServiceConfiguration ServiceConfiguration() => new(
            ...
            "0123456789abcdef0123456789abcdef",
            ExecutablePath,
            Path.Combine(LocalApplicationData, "CodeSignAuto", "agent.json"));
