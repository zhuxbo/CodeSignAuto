...
public sealed class SetupLocalizationTests
{
            ...
            File.WriteAllText(Path.Combine(root, "install.json"), """
                {
                  ...
                  "executablePath": "C:\\Program Files\\CodeSignAuto\\CodeSignAuto.exe"
                }
                """);
    ...
    public void Chinese_and_english_setup_resources_have_identical_nonempty_keys()
    {
        var manager = new ResourceManager(
            "CodeSignAuto.Setup.Resources.SetupStrings",
    ...
    [InlineData("zh-CN", "autologon_cleanup_owned_state", "CodeSignAuto", "不会清理", "错误代码：")]
    [InlineData("en-US", "autologon_cleanup_owned_state", "CodeSignAuto", "will not remove", "Error code: ")]
    ...
    public void Setup_form_switches_language_twice_without_starting_installation()
    {
        RunInSta(() =>
        {
            ...
            Assert.Equal("CodeSignAuto 安装", form.Text);
            ...
            Assert.Equal("CodeSignAuto Setup", form.Text);
