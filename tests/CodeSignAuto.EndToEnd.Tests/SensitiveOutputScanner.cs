...
internal static partial class SensitiveOutputScanner
{
    public static IReadOnlyList<string> Find(string text, params string[] exactSecrets)
    {
        ...
        AddIfMatch(findings, "authorization", AuthorizationPattern(), text);
        AddIfMatch(findings, "otpauth", OtpauthPattern(), text);
        AddIfMatch(findings, "otp", OtpPattern(), text);
        AddIfMatch(findings, "sid", SidPattern(), text);
        AddIfMatch(findings, "absolute_path", AbsolutePathPattern(), text);
        AddIfMatch(findings, "private_key", PrivateKeyPattern(), text);
        AddIfMatch(findings, "synthetic_marker", SyntheticMarkerPattern(), text);
        AddIfMatch(findings, "native_exception", NativeExceptionPattern(), text);
        if (exactSecrets.Any(secret => !string.IsNullOrEmpty(secret) && text.Contains(secret, StringComparison.Ordinal)))
        {
