...
public sealed class UninstallSafetyTests
{
    ...
    public void Windows_manual_activation_removal_preserves_other_manual_data()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto.ActivationUninstall", Guid.NewGuid().ToString("N"));
    ...
    public void Windows_manual_activation_removal_refuses_unsafe_targets_without_deleting_them()
    {
        foreach (var mismatch in new[] { "reparse", "hardlink", "owner-acl" })
        {
            var root = Path.Combine(
                Path.GetTempPath(),
                "CodeSignAuto.ActivationUninstall",
    ...
    public void Purge_manifest_round_trip_uses_value_equality_and_rejects_changed_quarantine_identity()
    {
        ...
        var exact = new PurgeQuarantineManifest(
            ...
            $"CodeSignAuto/v1/{operationId}",
            "S-1-5-21-1000-2000-3000-4000",
            operationId,
            UninstallSigningUserOwnership.ProductManaged,
            Path.GetFullPath(Path.Combine(Path.DirectorySeparatorChar.ToString(), "opt", "CodeSignAuto.exe")),
            new string('a', 64),
        ...
        Assert.All(
            new[]
            {
                roundTrip with { InstallOwnerMarker = $"CodeSignAuto/v1/{new string('f', 32)}" },
    ...
    public void Purge_manifest_binds_cleanup_to_the_installed_main_program_without_staging_its_directory()
    {
        ...
        var installedRoot = Path.Combine(sourceRoot, "CodeSignAuto");
        var installedExecutable = Path.Combine(installedRoot, "CodeSignAuto.exe");
    ...
    public void Purge_sources_always_include_the_installed_program_directory()
    {
        ...
        var programRoot = Path.Combine(root, "program-files", "CodeSignAuto");
        var executable = Path.Combine(programRoot, "CodeSignAuto.exe");
    ...
    public void Purge_manifest_rejects_a_manifest_location_inside_any_source_tree()
    {
        ...
        var plan = new PurgeIsolationPlan(
            ...
            manifestPath,
            $"CodeSignAuto.Purge.{operationId}",
            Path.Combine(sourceRoot, "CodeSignAuto.exe"),
            [new PurgeIsolationGroup(
                volumeRoot,
                stagingRoot,
    ...
    public void Purge_cleanup_task_is_local_system_at_startup_and_hidden_command_is_exact()
    {
        ...
        var plan = new PurgeIsolationPlan(
            ...
            $"CodeSignAuto.Purge.{operationId}",
            Path.GetFullPath(Path.Combine(Path.GetTempPath(), "CodeSignAuto.exe")),
            []);
    ...
    public void Purge_cleanup_task_runs_the_installed_main_program_then_removes_its_empty_install_tree()
    {
        ...
        var stagingRoot = Path.Combine(
            volumeRoot,
            $".CodeSignAuto.quarantine.{operationId}.0");
        var installedRoot = Path.Combine(volumeRoot, "Program Files", "CodeSignAuto");
        var installedExecutable = Path.Combine(installedRoot, "CodeSignAuto.exe");
        var manifestPath = PurgeQuarantineManifestCodec.ExpectedManifestPath(operationId);
        var plan = new PurgeIsolationPlan(
            operationId,
            ...
            [new PurgeIsolationGroup(
                ...
                stagingRoot,
                [new PurgeIsolationTarget(
                    Path.Combine(volumeRoot, "ProgramData", "CodeSignAuto"),
                    Path.Combine(stagingRoot, "program-data"))])]);
    ...
    public async Task Windows_accepts_and_round_trips_the_exact_purge_cleanup_task_xml()
    {
        ...
        var taskName = $"CodeSignAuto.Purge.{operationId}";
    ...
    public async Task Windows_schedules_and_rolls_back_the_exact_protected_purge_cleanup_state()
    {
        ...
        var executablePath = Path.Combine(executableDirectory, "CodeSignAuto.exe");
        ...
        var plan = new PurgeIsolationPlan(
            ...
            [new PurgeIsolationGroup(
                volumeRoot,
                Path.Combine(volumeRoot, $".CodeSignAuto.quarantine.{operationId}.0"),
                [new PurgeIsolationTarget(
                    executableDirectory,
                    Path.Combine(
                        volumeRoot,
    ...
    public void Completed_autologon_finalization_is_recognized_only_when_every_product_secret_and_receipt_is_absent()
    {
        ...
        Assert.False(WindowsAutoLogonPlatform.IsFullyFinalizedEvidence(
            ...
            lsaSecretPresent: false));
        Assert.False(WindowsAutoLogonPlatform.IsFullyFinalizedEvidence(
            ownerMarker: "CodeSignAuto/v1/0123456789abcdef0123456789abcdef",
            uninstallState: null,
            uninstallSid: null,
            uninstallAccount: null,
            uninstallProfile: null,
    ...
    public void Purge_autologon_finalization_accepts_only_manifest_exact_owner_sid_and_install()
    {
        ...
        const string ownerMarker = "CodeSignAuto/v1/0123456789abcdef0123456789abcdef";
        const string signingUserSid = "S-1-5-21-1000-2000-3000-4000";
        var exact = new DisabledAutoLogonFinalizationEvidence(
            ownerMarker,
            "disabled",
            signingUserSid,
            "MACHINE\\CodeSignAutoAgent",
            Path.GetFullPath(Path.Combine(Path.GetTempPath(), "CodeSignAutoAgent")),
            HasPriorState: true,
            DefaultPasswordPresent: false,
            LsaReceiptPresent: true,
        ...
        var wrongOwner = Assert.Throws<InstallException>(() =>
            WindowsAutoLogonPlatform.RequiresExactDisabledAutoLogonFinalization(
                ...
                exact with
                {
                    OwnerMarker = "CodeSignAuto/v1/fedcba9876543210fedcba9876543210",
                }));
    ...
    public void Interrupted_purge_discovery_accepts_only_one_exact_direct_non_reparse_operation_root()
    {
        ...
        var first = Path.Combine(programData, "CodeSignAuto.Purge.0123456789abcdef0123456789abcdef");
        var second = Path.Combine(programData, "CodeSignAuto.Purge.fedcba9876543210fedcba9876543210");
        var entries = new[]
        {
            new PurgeResumeCandidate(Path.Combine(programData, "CodeSignAuto.Purge.not-an-operation"), false),
    ...
    public void Interrupted_purge_manifest_requires_exact_owner_sid_install_executable_hash_and_source_paths()
    {
        ...
        var changed = new PurgeQuarantineManifest[]
        {
            ...
            {
                InstallOwnerMarker = "CodeSignAuto/v1/fedcba9876543210fedcba9876543210",
            ...
            fixture.Manifest with
            {
                ExecutablePath = Path.Combine(Path.GetDirectoryName(fixture.ExecutablePath)!, "other", "CodeSignAuto.exe"),
            },
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "CodeSignAuto.Uninstall.Purge", Guid.NewGuid().ToString("N"));
        var dataRoot = Path.Combine(root, "ProgramData", "CodeSignAuto");
        var agentRoot = Path.Combine(root, "Users", "signer", "CodeSignAuto");
    ...
    private static PurgeIsolationPlan ResumePlan()
    {
        ...
        var stagingB = Path.Combine(
            volumeB,
            $".CodeSignAuto.quarantine.{operationId}.1");
        return new PurgeIsolationPlan(
            operationId,
            Path.GetFullPath(Path.Combine(Path.GetTempPath(), "resume", operationId, "manifest.json")),
            $"CodeSignAuto.Purge.{operationId}",
            Path.GetFullPath(Path.Combine(Path.GetTempPath(), "resume", "CodeSignAuto.exe")),
            [
                new PurgeIsolationGroup(
                    volumeA,
                    stagingA,
                    [new PurgeIsolationTarget(
                        Path.Combine(volumeA, "CodeSignAuto"),
                        Path.Combine(stagingA, "program-data"))]),
                new PurgeIsolationGroup(
                    volumeB,
                    stagingB,
                    [new PurgeIsolationTarget(
                        Path.Combine(volumeB, "Users", "signer", "AppData", "Local", "CodeSignAuto"),
    ...
    private static ResumeManifestTestFixture ResumeManifestFixture()
    {
        ...
        var operationRoot = Path.Combine(programDataRoot, $"CodeSignAuto.Purge.{operationId}");
        var executablePath = Path.Combine(programDataRoot, "install", "CodeSignAuto.exe");
        ...
        var manifest = new PurgeQuarantineManifest(
            ...
            [stagingRoot],
            [new PurgeQuarantineTarget(
                Path.Combine(programDataRoot, "CodeSignAuto"),
                Path.Combine(stagingRoot, "program-data"))]);
        ...
        : IWindowsUninstallNative
    {
        ...
        public void VerifySigningIdentityOwnership(
            ...
            CodeSignAuto.Service.ServiceConfiguration configuration) => FailIf("identity");
    ...
    private sealed class WindowsOptionalToolTreeFixture : IDisposable
    {
        ...
        public WindowsOptionalToolTreeFixture()
        {
            Root = Path.Combine(
                Path.GetTempPath(),
                "CodeSignAuto.OptionalUninstall",
                Guid.NewGuid().ToString("N"));
            var dataRoot = Path.Combine(Root, "ProgramData", "CodeSignAuto");
            var productRoot = Path.Combine(Root, "Program Files", "CodeSignAuto");
            ToolsRoot = Path.Combine(Root, "Program Files", "CodeSignAuto PDF Support");
            HelperPath = Path.Combine(ToolsRoot, "CodeSignAutoPdfSigner.exe");
            ...
            Configuration = Configuration() with
            {
                ...
                ExecutablePath = Path.Combine(productRoot, "CodeSignAuto.exe"),
            };
        ...
        public CodeSignAuto.Service.ServiceConfiguration Configuration { get; }
    ...
    private static string OptionalExtensionRoot(
        CodeSignAuto.Service.ServiceConfiguration configuration)
    {
        ...
        return Path.Combine(programFilesRoot, "CodeSignAuto PDF Support");
    }

    private static string OptionalHelperPath(CodeSignAuto.Service.ServiceConfiguration configuration) =>
        Path.Combine(
            OptionalExtensionRoot(configuration),
    ...
    private static OptionalToolUninstallTreeSnapshot ExactOptionalToolTree() => new(
        [
            ...
            new OptionalToolUninstallTreeEntry("CodeSignAutoPdfSigner.exe", IsDirectory: false),
    ...
    private static PurgeInstallIdentity PurgeIdentity(
        ...
        new(
            $"CodeSignAuto/v1/{installInstanceId}",
            "S-1-5-21-1000-2000-3000-4000",
            installInstanceId,
            ownership,
            executablePath ?? Path.GetFullPath(Path.Combine(
    ...
    private static OwnedUserProfileReceipt ExactProfileReceipt(UninstallSigningIdentity identity) =>
        new(
            ...
            InstallInstanceId: identity.OwnerMarker["CodeSignAuto/v1/".Length..],
            Sid: identity.Sid,
            AccountName: identity.AccountName,
            RecordedPath: identity.ProfilePath);

    private static CodeSignAuto.Service.ServiceConfiguration Configuration()
    {
        var root = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "CodeSignAuto.Tests", "controlled"));
        return new CodeSignAuto.Service.ServiceConfiguration(
            new string('a', 64),
            "S-1-5-21-1000-2000-3000-4000",
            Path.Combine(root, "CodeSignAuto"),
            Path.Combine(root, "CodeSignAuto", "spool"),
            7080,
            "CodeSignAuto/v1",
            "0123456789abcdef0123456789abcdef",
            Path.Combine(root, "CodeSignAuto.exe"),
            Path.Combine(root, "user", "AppData", "Local", "CodeSignAuto", "agent.json"));
    }
    ...
    private static InstallationReceipt ManualReceipt()
    {
        ...
        return new InstallationReceipt(
            ...
            "0123456789abcdef0123456789abcdef",
            "S-1-5-21-1000-2000-3000-4000",
            Path.Combine(root, "Program Files", "CodeSignAuto", "CodeSignAuto.exe"),
            Path.Combine(root, "Users", "admin", "AppData", "Local", "CodeSignAuto", "manual"));
    }
        ...
        InstallationReceipt? receipt = null) : IUninstallEnvironment
    {
        ...
        public Task<CodeSignAuto.Service.ServiceConfiguration> LoadConfigurationAsync(
        ...
        public void ValidateControlledPaths(
            CodeSignAuto.Service.ServiceConfiguration configured)
        {
        }
