    ...
    public SetupPreflightResult Inspect()
    {
        ...
        try
        {
            ...
            var dataRoot = Path.GetFullPath(Path.Combine(
                ...
                "CodeSignAuto"));
            if (lookup.ServiceExists("CodeSignAuto.Service") ||
                lookup.TaskExists("CodeSignAuto.Agent") ||
...
{
    internal const string ManagedUserName = "CodeSignAutoAgent";
...
internal static class SetupAgentConfigurationFactory
{
        ...
        string programDataRoot)
    {
        ...
        var dataRoot = Path.GetFullPath(Path.Combine(programDataRoot, "CodeSignAuto"));
...
internal sealed class WindowsSetupResourceReservation : ISetupResourceReservation
{
    public string Reserve()
    {
        var programData = Path.GetFullPath(
            Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData));
        var dataRoot = Path.GetFullPath(Path.Combine(programData, "CodeSignAuto"));
        try
        {
...
internal sealed class TemporarySetupConfigurationWriter : ISetupConfigurationWriter
{
        ...
        CancellationToken cancellationToken)
    {
        var configuration = SetupAgentConfigurationFactory.Create(
            ...
            preflight,
            Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData));
        var instanceId = user.OwnerMarker!["CodeSignAuto/v1/".Length..];
        var path = Path.GetFullPath(Path.Combine(
            Path.GetTempPath(),
            $"CodeSignAuto.setup.{instanceId}.agent.json"));
        if (File.Exists(path))
        {
