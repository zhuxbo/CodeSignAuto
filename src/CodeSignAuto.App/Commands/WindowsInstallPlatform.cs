...
public sealed class WindowsInstallActionExecutor : IInstallActionExecutor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var xmlPath = Path.Combine(Path.GetTempPath(), $"CodeSignAuto-task-{Guid.NewGuid():N}.xml");
        try
        {
...
internal static class WindowsExecutableSecurity
{
    ...
    internal static bool IsRequiredLocation(string executablePath, string programFiles) =>
        ...
        string.Equals(
            ...
            Path.GetFullPath(Path.Combine(programFiles, "CodeSignAuto")),
            StringComparison.OrdinalIgnoreCase) &&
        string.Equals(
...
{
    internal const string InstallOwnerValueName = "CodeSignAutoInstallOwner";
...
{
    private const string InstallOwnerPrefix = "CodeSignAuto/v1/";
    internal const string KeyPath =
        @"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\CodeSignAuto";
    ...
    private static void ValidateRegistration(ProductUninstallRegistration registration)
    {
        ...
        if (!Path.IsPathFullyQualified(registration.ExecutablePath) ||
            !string.Equals(
                ...
                StringComparison.OrdinalIgnoreCase) ||
            !string.Equals(registration.DisplayName, "CodeSignAuto", StringComparison.Ordinal) ||
            string.IsNullOrWhiteSpace(registration.DisplayVersion) ||
            !string.Equals(registration.Publisher, "CodeSignAuto", StringComparison.Ordinal) ||
            !string.Equals(
...
internal static class WindowsTaskXml
{
    ...
    public static string Create(CreateInteractiveLogonTask action)
    {
        ...
        return new XDocument(
            ...
            new XElement(Ns + "Task",
                ...
                new XElement(
                    Ns + "RegistrationInfo",
                    new XElement(Ns + "Description", "CodeSignAuto/v1"),
        ...
        string ownerMarker)
    {
        ...
        return triggerChildren.All(child => child.Name == Ns + "Enabled" || child.Name == Ns + "UserId") &&
            ...
            SingleValue(registration, "Description") == "CodeSignAuto/v1" &&
