...
public sealed class ManualInstallPlanner(IManualInstallEnvironment environment)
{
    public ManualInstallPlan Plan()
    {
        ...
        var registration = ProductUninstallRegistration.Create(
            executablePath,
            CodeSignAuto.App.ApplicationVersion.ReadIdentity(typeof(ManualInstallPlanner).Assembly),
            marker);
        var paths = new ManualInstallPaths(
            dataRoot,
            Canonical(Path.Combine(dataRoot, "install.json")),
            Canonical(Path.Combine(commonDesktop, "CodeSignAuto.lnk")),
            executablePath);
...
public sealed class InstallPlanner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var dataRoot = Canonical(Path.Combine(_environment.ProgramDataRoot, "CodeSignAuto"));
        ...
        var productRegistration = ProductUninstallRegistration.Create(
            ...
            CodeSignAuto.App.ApplicationVersion.ReadIdentity(typeof(InstallPlanner).Assembly),
            exactOwnerMarker);
        ...
        actions.Add(new VerifyInstallSecurity(
            ...
            options.ListenPort,
            exactOwnerMarker));
        actions.Add(new StartAndVerifyWindowsService("CodeSignAuto.Service", exactOwnerMarker));
