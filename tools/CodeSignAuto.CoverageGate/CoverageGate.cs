...
public static class CoverageGate
{
    ...
    private static readonly IReadOnlyDictionary<string, int> Thresholds =
        ...
        {
            ["CodeSignAuto.Core"] = 8_500,
            ["CodeSignAuto.Service"] = 8_500,
            ["CodeSignAuto.Protocol"] = 8_500,
            ["CodeSignAuto.Agent"] = 7_500,
            ["CodeSignAuto.UI.ViewModels"] = 8_000,
        };
        ...
        IReadOnlyDictionary<string, Dictionary<SourceLine, bool>> linesByTarget)
    {
        ...
        foreach (var package in packages.Elements("package"))
        {
            ...
            var exactTarget = linesByTarget.ContainsKey(name) && name != "CodeSignAuto.UI.ViewModels"
                ? name
                : null;
            var uiPackage = string.Equals(name, "CodeSignAuto", StringComparison.Ordinal);
            ...
            foreach (var @class in classes.Elements("class"))
            {
                ...
                var uiFileName = uiPackage
                    ? NormalizeSourcePath(suppliedFileName, "CodeSignAuto.App")
                    : null;
                if (uiFileName is not null &&
                    uiFileName.StartsWith("CodeSignAuto.App/UI/", StringComparison.Ordinal) &&
                    uiFileName.EndsWith("ViewModel.cs", StringComparison.Ordinal))
                {
                    AddClassLines(@class, uiFileName, linesByTarget["CodeSignAuto.UI.ViewModels"]);
                }
            }
        }
    }
